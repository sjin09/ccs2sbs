...
def cs2subindel(read):
    ...
    for cstuple in read.cstuple_lst:
        ...
        if state == 2:  # snp 
            qbq_lst.append(read.bq_int_lst[qpos])
            tsub_lst.append((read.tname, tpos + 1, ref, alt))
            qsub_lst.append((read.qname, qpos + 1, alt, ref))
        elif state == 3 or state == 4:  # insertion
            tindel_lst.append(read.tname, tpos, ref, alt)
        tpos += ref_len 
        qpos += alt_len
    return tsub_lst, qsub_lst, qbq_lst, tindel_lst
